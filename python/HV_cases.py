...
plot_colors['Coev. Penalty'] = '#56B4E9'
...
## Sample NFEs
nfe_samples = []
#max_nfe = 5000 # for partitioning
max_nfe = 10800 # for partitioning
if problem_dir == 'Artery\\' or problem_dir == 'Equal Stiffness\\':
    #max_nfe = 6000
    max_nfe = 13500
elif problem_dir == 'Assign\\':
    max_nfe = 27000
...
for case_key in list(case_bools.keys()):
    ...
    for i in range(n_runs):
        ...
        if (heurs_dir == 'Epsilon MOEA\\') or (heurs_dir == 'AOS\\'): # Epsilon MOEA or AOS - just read csv file for corresponding run
            current_filepath = results_dir + problem_dir + heurs_dir
            
            if heurs_dir == 'Epsilon MOEA\\':
            ...
            for n in range(len(coev_nfe_vals)):
                ...
                allzeros = False
                if first_wts_true_zero:
                    if np.all(heur_weight == -3) and (problem_dir == 'Artery\\' or problem_dir == 'Equal Stiffness\\'): # for real valued weight decisions, initial set of weights is 0.0
                        heur_weight = np.full((heur_weight.shape[0]), 0.0)
                        allzeros = True
                ...
                for j in range(len(heuristic_names)):
                    if int_weights:
                        ...
                        if not allzeros:
                            ...
                            weight_val = heur_weight[j+start_idx]
                ...
                if weight_of_weights:
                    if weight_mult < 0.99e-3 and weight_mult > 0:
...
if problem_dir == 'Assign\\':
    ...
    nfe_start = 0
    nfe_end = 1000
elif  problem_dir == 'Artery\\':
    nfe_start = 10000   
    nfe_end = 13500


if problem_dir == 'Assign\\' or problem_dir == 'Partition\\' or problem_dir == 'Artery\\':
