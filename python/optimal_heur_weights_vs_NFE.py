...
problem_dir = 'Artery\\' # Equal Stiffness, Artery ,Assign, Partition, (C1_DTLZ1, Simple DTLZ1_2, UF1 are test problems)
...
if problem_dir == 'Equal Stiffness\\' or problem_dir == 'Artery\\':
...
# Get NFE values
filepath = results_dir + problem_dir + 'Coevolutionary\\'+ coev_dir + 'run 0\\'
...
for j in range(len(nfe_vals)):
    ...
    for i in range(n_runs):
        current_filepath = results_dir + problem_dir + 'Coevolutionary\\' + coev_dir + 'run ' + str(i) + '\\'
        ...
        if -fitness[closest_idx] > 0.0:
            ...
            for k in range(len(heuristic_names_full)):
                if weight_of_weights:
                    ...
                    optimal_heur_weights_nfe[k,i] = 10**-3 # 10^-3 is the lowest weight used in the simulations
          
        if weight_of_weights:
            if j == 0:
                windowed_weight_mults_nfe[i] = alpha*optimal_weight_mults_nfe[i]
            else:
                windowed_weight_mults_nfe[i] = (1 - alpha)*windowed_weight_mults['nfe:'+str(nfe_vals[j-1])][i] + alpha*optimal_weight_mults_nfe[i]
        for k in range(len(heuristic_names_full)):
            if j == 0:
                windowed_weights_nfe[k,i] = alpha*optimal_heur_weights_nfe[k,i]
            else:
                windowed_weights_nfe[k,i] = (1 - alpha)*windowed_heur_weights[heuristic_names_full[k]]['nfe:'+str(nfe_vals[j-1])][i] + alpha*optimal_heur_weights_nfe[k,i]
...
for i in range(len(nfe_vals)):
    for heur_name in heuristic_names_full:
        ...
        #windowed_heur_weights_current_med = np.mean(windowed_heur_weights[heur_name]['nfe:'+str(nfe_vals[i])]) 
        if with_IQR:
            ...
            #windowed_heur_weights_current_std = np.std(windowed_heur_weights[heur_name]['nfe:'+str(nfe_vals[i])])
            #windowed_heur_weights_current_1q = np.add(windowed_heur_weights_current_med, np.multiply(windowed_heur_weights_current_std, -2))
            #windowed_heur_weights_current_3q = np.add(windowed_heur_weights_current_med, np.multiply(windowed_heur_weights_current_std, -2))
...
for heur_name in heuristic_names_full:
    ...
    heur_counter += 1
plt.xlabel('NFE')
plt.ylabel('Cumulative Heuristic Weight')
...
if weight_of_weights:
    ...
    for i in range(len(nfe_vals)):
        ...
        #windowed_weight_mults_med[i] = np.mean(windowed_weight_mults['nfe:'+str(nfe_vals[i])])
        if with_IQR:
            ...
            #windows_weight_mults_std = np.std(windowed_weight_mults['nfe:'+str(nfe_vals[i])])
            #windowed_weight_mults_1q[i] = np.add(windowed_weight_mults_med[i], np.multiply(windows_weight_mults_std, -2))
            #windowed_weight_mults_3q[i] = np.add(windowed_weight_mults_med[i], np.multiply(windows_weight_mults_std, 2))
    ...
    plt.xlabel('NFE')
    plt.ylabel('Cumulative Heuristic Weight Multiplier')
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
